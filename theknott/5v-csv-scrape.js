...
// Function to clean invalid emails (starting with 'n' fo llowed by capital letter)
...
(async () => {
  try {
        ...
        for (const link of links) {
          try {
            ...
            if (websiteLink !== "N/A") {
              ...
              uniqueEmailAddresses = uniqueEmailAddresses.join(", ");
            } else {
              console.log(
            ...
            let vendorState = "";
            try {
              fullLocation = locationKeys[locationIndex];
              fullLocation = fullLocation.split(",");
              vendorCity = fullLocation[0].trim();
              vendorState = fullLocation[1].trim();
            } catch {}
            ...
            const vendorInfo = {
              ...
              address: address || "N/A",
              city: vendorCity || city || "N/A", // Add the city
              state: vendorState || state || "N/A", // Add the state
    ...
    // Save the vendor data to a JSON file
    const jsonFilePath = `${process.cwd()}/vendorData_${timestamp}.json`;
    fs.writeFile(jsonFilePath, JSON.stringify(vendorData, null, 2), (err) => {
      if (err) {
        console.error("Error saving the vendor data to JSON file:", err);
      } else {
        console.log(`Vendor data successfully saved to ${jsonFilePath}`);
      }
    });

    // Convert the vendor data to CSV format
    ...
    // Save the vendor data to a CSV file
    const csvFilePath = `${process.cwd()}/vendorData_${timestamp}.csv`;
    fs.writeFile(csvFilePath, csvData, (err) => {
      if (err) {
        console.error("Error saving the vendor data to CSV file:", err);
      } else {
        console.log(`Vendor data successfully saved to ${csvFilePath}`);
      }
    });
  } catch (error) {
