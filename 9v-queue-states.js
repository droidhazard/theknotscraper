...
// Function to clean invalid emails
...
(async () => {
  try {
    ...
    for (const [locationName, locationSlug] of Object.entries(locations)) {
      console.log(`Starting extraction for location: ${locationName}`);

      // Loop through categories for each location
      for (const selectedCategory of categories) {
        let page = 1;
        let hasMoreVendors = true;

        while (hasMoreVendors) {
          const url = `https://www.theknot.com/marketplace/${selectedCategory}-${locationSlug}?sort=featured&page=${page}`;
          console.log(
            `Fetching page ${page} for category "${selectedCategory}" and location "${locationName}": ${url}`
          );
          ...
          try {
            ...
            if (links.length === 0) {
              console.log(
                `No more entries on page ${page} for category "${selectedCategory}" and location "${locationName}".`
              );
            ...
            for (const link of links) {
              try {
                ...
                const guestCapacity = $vendorPage(
                  ...
                  .text()
                  .trim();
                const address = $vendorPage("span.link-underline--333b0")
                  .text()
                  .trim();
                const { city, state } = extractCityState(locationName);
                ...
                if (websiteLink !== "N/A") {
                  const emailsFound =
                    $vendorPage.text().match(emailRegex) || [];
            ...
            console.error(
              `Error fetching page ${page} for category "${selectedCategory}" and location "${locationName}": ${pageError.message}`
            );
            hasMoreVendors = false;
          }
        }
